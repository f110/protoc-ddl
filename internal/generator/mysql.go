...
func (m MySQLGenerator) Generate(writer *bytes.Buffer, tables []*schema.Table) {
...
func (m MySQLGenerator) generateColumnDefinition(table *schema.Table) []string {
	colLine := make([]string, 0)
...
func (MySQLGenerator) generateIndexDefinition(table *schema.Table) []string {
	colLine := make([]string, 0)
...
func (MySQLGenerator) columnType(col schema.Column) string {
	columnType := ""
